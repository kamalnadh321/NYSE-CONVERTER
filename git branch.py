...
def process_files(src_base_dir, ds, tgt_base_dir):
    for file in glob.glob(f'{src_base_dir}/part*'):
    ...
    if not datasets:
        for path in glob.glob(f'{src_base_dir}/*'):
            if os.path.isdir(path):
                process_files(src_base_dir, os.path.split(path)[1], tgt_base_dir)
    else:
        dirs = datasets.split(',')
        for dis in dirs:
            process_files(src_base_dir, ds , tgt_base_dir )
